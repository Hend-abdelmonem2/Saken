...
{
    public class GetHousingByIdHandler : BaseHousingHandler,IRequestHandler<GetHousingByIdQuery, BaseResponse<HousingDetailsDto?>>
    {

        public GetHousingByIdHandler(IHousingService service) :base(service) { }

        public async Task<BaseResponse<HousingDetailsDto?>> Handle(GetHousingByIdQuery request, CancellationToken ct)
